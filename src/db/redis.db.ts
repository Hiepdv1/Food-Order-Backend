...
const handleEventConnection = () => {
    redisClient.on(statusConnectRedis.CONNECT, () => {
        logger.info("connectionRedis - Connection status: connected successfully");
    });
    ...
    redisClient.on(statusConnectRedis.RECONNECT, () => {
        logger.info("connectionRedis - Connection status: Redis is reconnecting");
    });
    redisClient.on(statusConnectRedis.ERROR, (err) => {
        logger.error(`Error connecting to Redis, Please try again later. ${err}`);
    });
};
